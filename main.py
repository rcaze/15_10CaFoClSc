...
# Creabbte the folders if not existing
...
def fig_binary_simple():
    """Generate the first figure of the article"""
    # Configure a stimulation object
    stim = lib.Stimulation()
    stim.n_neurons = 200
    stim.group_number = 2
    stim.group_size = 100

    # Configure the binary neuron model
    neuron = lib.BinaryModel()
    neuron.synapses = lib.syn_gen()
    neuron.thresholds = [60, 40, 40]
    neuron.d_spike = 60

    # Generate the spikes and their processing
    stim()
    neuron(stim.spikes)

    # Plot the subfigures
    stim.show(save="%sFig1a%s" % (FIG_FOLDER, FIG_SUF))
    plot.integration([0, 1], neuron, ymax=100,
                     save="%sFig1c%s" % (FIG_FOLDER, FIG_SUF))
    plot.integration([0, 1], neuron, ymax=100, soma_th=0.6,
                     save="%sFig1e%s" % (FIG_FOLDER, FIG_SUF))

    stim.shift_spikes(100)
    neuron(stim.spikes)
    stim.show(save="%sFig1b%s" % (FIG_FOLDER, FIG_SUF))
    plot.integration([0, 1], neuron, ymax=100,
                     save="%sFig1d%s" % (FIG_FOLDER, FIG_SUF))
    plot.integration([0, 1], neuron, ymax=100, soma_th=0.6,
                     save="%sFig1f%s" % (FIG_FOLDER, FIG_SUF))
...
def fig_stim_sel(NREP=10):
    ...
    rbiass = lib.scanpar(NREP, pars, par_name="b")
    ...
    sep = lib.results_noise(NREP, frac_fails)
    ...
    rclusts = lib.scanpar(NREP, pars_c)
    FIG_NAME = "%sFig%dA%s" % (FIG_FOLDER, FIG_N, FIG_SUF)
    plot.scan_plot(rclusts, pars_c, xlab="Clustering", save=FIG_NAME)

    # Removing dendrites
    sep, sep_l, dists = lib.results_dend_rem(NREP)
